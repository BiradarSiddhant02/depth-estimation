...
def ARE(output, truth):
    ...
    for i in range(H):
        for j in range(W):
            if truth[i, j] != 0: 
                total_error += abs((truth[i, j] - output[i, j]) / truth[i, j])
...
with torch.no_grad():
    for batch in test_loader:
        ...
        model_0_rel = ARE(output_0, depth_n)      
        model_1_rel = ARE(output_1, depth_n)      
        model_2_rel = ARE(output_2, depth_n)      
        model_3_rel = ARE(output_3, depth_n)

        print(model_0_rel, model_1_rel, model_2_rel, model_3_rel)      
